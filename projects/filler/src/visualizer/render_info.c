...
/*   Updated: 2019/09/23 15:42:25 by cormund          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ren_info(t_vis *vis, t_game *game)
{
	...
	TTF_SizeText(vis->font_text, game->size_board.x < 42 ? "24X40" : "100X99",\
															&sz2.x, &sz2.y);
	indent.x = vis->up_win[1].w / 2 - sz.x;
	SDL_RenderCopy(vis->ren, vis->info[0], NULL, &(SDL_Rect){vis->up_win[1].x\
				+ indent.x, vis->up_win[1].y + sz.y * 2 - 5, sz.x + sz2.x,\
																		sz.y});
...
void			ren_cntrl_key(t_vis *vis)
{
	...
	indent = vis->up_win[2].w / 2 - sz.x;
	TTF_SizeText(vis->font_text, PLAY_PAUSE, &sz.x, &sz.y);
	SDL_RenderCopy(vis->ren, vis->cntrl_key[0], NULL, &(SDL_Rect){\
	vis->up_win[2].x + indent, vis->up_win[2].y + sz.y * 2 - 5, sz.x, sz.y});
	TTF_SizeText(vis->font_text, "Prev/next  :  ", &sz.x, &sz.y);
	indent = vis->up_win[2].w / 2 - sz.x;
	TTF_SizeText(vis->font_text, PREV_NEXT, &sz.x, &sz.y);
	SDL_RenderCopy(vis->ren, vis->cntrl_key[1], NULL, &(SDL_Rect){\
	vis->up_win[2].x + indent, vis->up_win[2].y + sz.y * 3 - 5, sz.x, sz.y});
	TTF_SizeText(vis->font_text, "Edir color  :  ", &sz.x, &sz.y);
	indent = vis->up_win[2].w / 2 - sz.x;
	TTF_SizeText(vis->font_text, EDIT_CLR, &sz.x, &sz.y);
	SDL_RenderCopy(vis->ren, vis->cntrl_key[2], NULL, &(SDL_Rect){\
	vis->up_win[2].x + indent, vis->up_win[2].y + sz.y * 4 - 5, sz.x, sz.y});
	TTF_SizeText(vis->font_text, "Speed  :  ", &sz.x, &sz.y);
	indent = vis->up_win[2].w / 2 - sz.x;
	TTF_SizeText(vis->font_text, SPEED, &sz.x, &sz.y);
	SDL_RenderCopy(vis->ren, vis->cntrl_key[3], NULL, &(SDL_Rect){\
	vis->up_win[2].x + indent, vis->up_win[2].y + sz.y * 5 - 5, sz.x, sz.y});
}
...
void			ren_fin(t_vis *vis, t_game *game)
{
	...
	SDL_RenderCopy(vis->ren, tex, NULL, &(SDL_Rect){(vis->fin_win.w - sz.x) /\
	2 + vis->fin_win.x, (vis->fin_win.h - sz.y) / 2 + vis->fin_win.y, sz.x,\
																		sz.y});
	free(text);
	SDL_DestroyTexture(tex);
}
